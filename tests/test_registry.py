...
from bio_trend.registry import JournalRegistry, RegistryError
...
def test_load_parses_journals_and_feeds(tmp_path):
    _write(tmp_path, [
        {"key": "cell", "label": "Cell", "family": "Cell Press",
         "feeds": [{"url": "https://x/cell.rss", "type": "current", "bio_focus": "high"}]},
    ])
...
def test_real_config_is_loadable():
    # the committed config/journals.json must parse and cover all three families
    reg = JournalRegistry.load()
    families = {j.family for j in reg.journals}
    assert {"Nature", "Science", "Cell Press"} <= families
